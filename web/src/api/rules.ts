...
export const getRuleList = (params: PageParams & {
  ...
  return api.get<ApiResponse<PageResponse<Rule>>>('/rules/list', { params })
}
...
export const importRuleConfig = (file: File) => {
  ...
  return api.post<ApiResponse<{ success: number; failed: number }>>('/rules/import', formData, {
    headers: {
...
export const exportRuleConfig = (ids?: number[]) => {
  return api.post<Blob>('/rules/export', { ids }, {
    responseType: 'blob'
  })
}
